// =======================
// Dữ liệu trung tâm HKD
// =======================
window.hkdData = {};

// =======================
// Khởi tạo / kiểm tra HKD
// =======================
...
// =======================
// Kiểm tra tồn tại MSP
// =======================
function isProductCodeExist(taxCode, code){
  const stocks = ['tonkhoMain','tonkhoKM','tonkhoCK'];
  return stocks.some(stock => hkdData[taxCode][stock]?.some(item => item.productCode===code));
}
...
function parseXmlInvoice(xmlContent){
  ...
  productNodes.forEach((node,idx)=>{
    ...
    const msp = code || generateProductCodeByName(buyerInfo.taxCode, category, name);
  ...
  const totals = {
    beforeTax: +products.reduce((s,p)=>s+p.amount,0).toFixed(2),
    tax: +products.reduce((s,p)=>s+(p.amount*p.taxRate/100),0).toFixed(2)
  };
  totals.total = +(totals.beforeTax + totals.tax).toFixed(2);
...
async function handleZipFiles(files){
  for(const file of files){
    ...
    try{
      ...
      if(!invoice||!invoice.products||invoice.products.length===0){logAction(`Bỏ qua file ${file.name}`); continue;}
      const taxCode = invoice.buyerInfo.taxCode || 'UNKNOWN';
      ensureHkdData(taxCode);

      const uniqueKey=`${invoice.invoiceInfo.symbol}_${invoice.invoiceInfo.number}`;
      if(isDuplicate(invoice,taxCode)){logAction(`Trùng HĐ, bỏ qua: ${uniqueKey}`); continue;}

      const computedTotal = +invoice.products.reduce((s,p)=>s+p.amount,0).toFixed(2) +
                            +invoice.products.reduce((s,p)=>s+(p.amount*p.taxRate/100),0).toFixed(2);
      ...
      logAction(`[NHẬP HĐ] MST=${taxCode}, HĐ=${uniqueKey}, trạng thái=${invoice.status.validation}`);
    } catch(err){
      logAction(`Lỗi file ${file.name}: ${err.message}`);
    }
  }
}
