...
function parseXmlInvoice(xmlContent) {
  ...
  productNodes.forEach((node,idx)=>{
    ...
    let category='hang_hoa'; const lower=name.toLowerCase(); const isCK=lower.includes('chiết khấu')||lower.includes('ck'); if(quantity===0&&price===0) category='KM'; else if(tchat===3&&isCK) category='chiet_khau';
...
async function handleZipFiles(files){
  for(const file of files){
    ...
    if(!invoice||!invoice.products||invoice.products.length===0){console.warn('Bỏ qua:',file.name); continue;}
    const taxCode=invoice.buyerInfo.taxCode||'UNKNOWN';
    ensureHkdData(taxCode);
    const uniqueKey=`${invoice.invoiceInfo.symbol}_${invoice.invoiceInfo.number}`;
    if(isDuplicate(invoice,taxCode)){console.log('Trùng HĐ, bỏ qua:',uniqueKey); continue;}
