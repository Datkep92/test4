...
function parseXmlInvoice(xmlContent){
  ...
  productNodes.forEach((node,idx)=>{
    ...
    let category='hang_hoa'; const lower=name.toLowerCase(); const isCK=lower.includes('chiết khấu')||lower.includes('ck'); 
    if(quantity===0&&price===0) category='KM'; else if(tchat===3&&isCK) category='chiet_khau';
  ...
  const totals={beforeTax:products.reduce((s,p)=>s+p.amount,0),
...
async function handleZipFiles(files){
  for(const file of files){
    ...
    try{
      ...
      if(!invoice||!invoice.products||invoice.products.length===0){window.logAction(`Bỏ qua file ${file.name}`); continue;}
